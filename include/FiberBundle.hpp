...
class FiberBundle {
	public:
		...
		FiberBundle(const FiberBundle & rhs);
		FiberBundle & operator=(const FiberBundle & rhs);
		...
		inline double TinK(const size_t i){return Tbase_K + (Tmax_K-Tbase_K)*std::exp(-std::pow(std::abs(zvals[i]-thermalcenter)/thermaldiam,int(2)));}
